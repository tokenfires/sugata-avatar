/** Production forest WebGPU integration against independent two-domain canonical goldens.
 * node packages/core/src/Avatar.hair-forest.gpu.selftest.mjs [--report=/tmp/report.json]
...
const goldenFile=path.join(here,'motion/fixtures/avatar-hair-forest-canonical-v2.json');assert.equal(fileSha(goldenFile),'50fb7c7380cf4964e0dc734772aae874f62083a40b1e6b4ceee53eced80f85ed');
...
const server=await startProbeServer({port:5284});let browser,page,replacements=0;const assetReads=[];
try{
 ...
 check('same-renderer contact/bob02/off and identity rebuilds own exactly35/8/0 storage buffers without accumulation',()=>{const r=report.runtime;assert.deepEqual(r.baseline,{storage:0,bytes:0,compute:0});assert.equal(r.sequence.length,7);for(const row of r.sequence){const expected=row.style===null?0:row.contact?35:8;assert.equal(row.resources.storage,expected,JSON.stringify(row));if(row.contact){assert.equal(row.contact.activeChains,496);assert.equal(row.resources.bytes,7177952);assert.equal(row.contact.calibration,golden.provenance.calibrationId);assert.equal(row.contact.forest.orderedTriangleNamespace,'global forest triangle address');}}const contactRows=r.sequence.filter(x=>x.contact);for(const row of contactRows)assert.deepEqual(row.resources,contactRows[0].resources);});
