/** Nearest point/whole-segment queries over independently bounded moving surface domains.
 ...
 * Caller owns the validated CPU patches and the renderer; query owns only its buffers.
 */
import {
...
const triangleBarycentric = Fn( ( [ point, a, b, c ] ) => {
    ...
    name: 'hairForestTriangleBarycentric', type: 'vec3', inputs: [
...
const segmentParameters = Fn( ( [ a, b, c, d ] ) => {
    ...
    return result;
} ).setLayout( { name: 'hairForestSegmentParameters', type: 'vec2', inputs:
...
const segmentTriangleCoordinates = Fn( ( [ p, q, a, b, c ] ) => {
    ...
    return result;
} ).setLayout( { name: 'hairForestSegmentTriangleCoordinates', type: 'vec4', inputs:
    [ 'p', 'q', 'a', 'b', 'c' ].map( name => ( { name, type: 'vec3' } ) ) } );

/** Isolated forest control. Inputs remain borrowed; only these three GPU buffers are owned.
 * Ordered triangle IDs are global forest addresses. Source IDs remain canonical body ordinals.
 * forChain must be invoked while building a TSL Fn with a validated immutable chain mapping.
 */
export function createSurfaceForestQuery( domains, { chainCount } ) {
    const packed = packForest( domains, { chainCount } );
    const vertexCount = packed.V, nodeCount = packed.N;
    const maximumDomainNodes = Math.max( ...packed.domains.map( d => d.nodeEnd - d.nodeStart ) );
    const vertices = instancedArray( packed.vertices, 'vec4' );
    const bounds = instancedArray( packed.bounds, 'vec4' );
    const topology = instancedArray( packed.topology, 'uvec4' );
    const buffers = [ vertices, bounds, topology ];
    let disposed = false;
    const requireLive = () => { if ( disposed ) throw Error( 'Surface forest query is disposed.' ); };
    const forChain = ( chain, scopeAlpha = float( 1 ) ) => {
        requireLive();
        if ( typeof chain === 'number' && ( !Number.isInteger( chain ) || chain < 0 || chain >= chainCount ) ) throw Error( 'Invalid forest chain.' );
        const range = topology.element( uint( chain ).add( uint( packed.chainHeadersOffset ) ) ).toVar();
        return {
        ...
        mayOverlapSegment( a, b, radius ) {
            ...
            const low = bounds.element( range.x ).xyz.toVar();
            const high = bounds.element( range.x.add( uint( nodeCount ) ) ).xyz.toVar();
        ...
        query( point, seedTriangle = uint( 0xffffffff ), alpha = scopeAlpha ) {
            if ( disposed ) throw new Error( 'Surface query is disposed.' );
            ...
            const node = range.x.toVar(), nodesVisited = uint( 0 ).toVar(), trianglesTested = uint( 0 ).toVar();
            ...
            If( seedTriangle.greaterThanEqual( range.z ).and( seedTriangle.lessThan( range.w ) ), () => {
                considerTriangle( seedTriangle );
            } );
            Loop( { start: 0, end: maximumDomainNodes, type: 'uint' }, () => {
                If( node.greaterThanEqual( range.y ), () => Break() );
        ...
        querySegment( segmentStart, segmentEnd, seedTriangle = uint( 0xffffffff ), alpha = scopeAlpha ) {
            if ( disposed ) throw new Error( 'Surface query is disposed.' );
            const positionAt = index => vertices.element( index.add( uint( vertexCount * 2 ) ) ).xyz.mul( float( 1 ).sub( alpha ) ).add( vertices.element( index ).xyz.mul( alpha ) );
            const normalAt = index => vertices.element( index.add( uint( vertexCount * 3 ) ) ).xyz.mul( float( 1 ).sub( alpha ) ).add( vertices.element( index.add( uint( vertexCount ) ) ).xyz.mul( alpha ) );
            const bestDistanceSquared = float( 1e30 ).toVar();
    ...
    return {
        buffers, forChain,
        layout: { vertices: packed.V, nodes: packed.N, triangles: packed.T, chainHeaders: chainCount,
            chainHeadersOffset: packed.chainHeadersOffset, orderedTriangleNamespace: 'global forest triangle address',
            bytes: { vertices: packed.vertices.byteLength, bounds: packed.bounds.byteLength, topology: packed.topology.byteLength },
            domains: packed.domains.map( ( { id, vertexStart, vertexCount, nodeStart, nodeEnd, triangleStart, triangleEnd } ) =>
                ( { id, vertexStart, vertexCount, nodeStart, nodeEnd, triangleStart, triangleEnd } ) ) },
        update() {
            requireLive(); packed.update();
            vertices.value.needsUpdate = true; bounds.value.needsUpdate = true;
        },
        // Default methods retain the existing call shape and scope to the domain of chain0.
        query( point, seedTriangle = uint( 0xffffffff ), alpha = float( 1 ) ) {
            return forChain( uint( 0 ), alpha ).query( point, seedTriangle );
        },
        querySegment( a, b, seedTriangle = uint( 0xffffffff ), alpha = float( 1 ) ) {
            return forChain( uint( 0 ), alpha ).querySegment( a, b, seedTriangle );
        },
        mayOverlapSegment( a, b, radius ) { return forChain( uint( 0 ) ).mayOverlapSegment( a, b, radius ); },
        dispose( renderer ) {
            ...
            try { packed.dispose(); } catch ( error ) { errors.push( error ); }
            if ( errors.length === 1 ) throw errors[ 0 ];
            if ( errors.length > 1 ) throw new AggregateError( errors, 'Surface forest query disposal failed.' );
        }
    };
}
