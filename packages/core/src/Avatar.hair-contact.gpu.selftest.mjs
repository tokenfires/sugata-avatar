/** Actual WebGPU integration: canonical nod equivalence, bind-aware roots and resource lifetime.
...
const sourceFiles=['Avatar.js','Avatar.hair-contact.browser.mjs','Avatar.hair-contact.gpu.selftest.mjs',...['HairDynamics','HairSkinTransform','HairSurface','HairSurfaceQuery','HairSurfaceContact','HairBodyContact','HairBodyContactCalibration','HairBodyContactCalibration.data'].map(n=>'motion/'+n+'.js')];
const sourceHashes=()=>Object.fromEntries(sourceFiles.map(file=>[file,fileSha(here+file)]));
const report={date:new Date().toISOString(),sourceHashes:sourceHashes(),candidateSha256:sha(candidate.bytes),goldenSha256:fileSha(goldenFile),checks:[],errors:[],loadedAssets:[],limits:['The portrait UI still reloads when switching hairstyles; this probes the runtime attachment boundary on one renderer, not a material-prototype uninstall.','Canonical equality covers the saved nod at0 and0.1seconds only. Root/native checks cover root centres, rebuilt midpoints and widths; transported edge positions can differ from static skinning. No dynamic whole-hair rigid equivariance or contact convergence claim.','Open neck/shoulder contact is not all-body, scalp, or garment clearance; no performance claim.']};
