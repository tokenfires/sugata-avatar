...
try{
 ...
 check('casual preset attaches real g050 body, bob02, foundation, suit and shoes on one canvas',()=>{const r=report.initial;assert.equal(r.canvasCount,1);assert.equal(r.report.identity.bake,'figure_g050');assert.equal(r.configuration.hair,'bob02');assert.deepEqual(r.configuration.wardrobe,{outfit:[...SHOWCASE_OUTFITS.casual.garments],foundation:{...SHOWCASE_FOUNDATION}});assert.equal(r.report.hair.attached,true);assert.equal(r.report.wardrobe.attached,true);});
 ...
 check('actual shipped g050 hair bytes load and browser console/network/page errors remain empty',()=>{for(const hair of ['bob01','bob02'])assert.ok(report.loadedAssets.some(a=>a.sha256===report.sourceHashes[`assets/hair/${hair}/g050.glb`]),hair+' asset digest');assert.deepEqual(report.errors,[]);});
