...
check('contradictory, repeated, unknown and unsupported URL selections fail rather than mislabel a render', () => {
    for (const query of ['style=ecru&style=charcoal','style=unknown','preset=casual&preset=elegant','outfit=bad','frame=neck','light=void','gender=0','bake=g100','hair=bob01','foundation=none'])
...
function report() { return {disposal:{disposed:false},identity:{gender:.5,bake:'figure_g050'},hair:{style:'bob02',loadedStyle:'bob02',attached:true},wardrobe:{attached:true,appearance:{style:'ecru',attached:true},pendingCandidates:0,foundation:{...SHOWCASE_FOUNDATION},requestedOutfit:['female_elegantsuit01','shoes01'],state:{worn:[...SHOWCASE_OUTFITS.casual.garments,...Object.values(SHOWCASE_FOUNDATION)]}},framing:{mode:'body'},scene:{id:'studio',lighting:{look:'studio'}},quality:{requested:'balanced'},motion:{seed:20260807}}; }
...
check('export refuses partial, retired, mismatched or unsupported actual runtime states', () => {
    for (const mutate of [r=>r.wardrobe.appearance.attached=false,r=>r.wardrobe.appearance.style='unknown',r=>r.disposal.disposed=true,r=>r.identity.bake='figure_g025',r=>r.hair.attached=false,r=>r.hair.loadedStyle='bob01',r=>r.wardrobe.attached=false,r=>r.wardrobe.pendingCandidates=1,r=>r.wardrobe.state.worn.pop(),r=>r.wardrobe.state.worn.push('fedora01'),r=>r.framing.mode='neck',r=>r.scene.lighting.look='cool']) {
