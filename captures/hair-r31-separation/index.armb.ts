//
// index.armb.ts — ARM B: OUR groom in THEIR renderer. index.control.ts plus three env knobs
// (HAIRFILE, FIBER, MESHES); everything else is index.control.ts's and unchanged.
...
// --- arm B's three additions ------------------------------------------------------------
const HAIRFILE = Deno.env.get('HAIRFILE');
if (HAIRFILE) CONFIG.hairFile = HAIRFILE as never;
CONFIG.hairRender.fiberRadius = num('FIBER', CONFIG.hairRender.fiberRadius);
const DROP_MESHES = Deno.env.get('MESHES') === '0';
...
async function renderSceneToFile(
  ...
  console.log(
    `Result: '${outputPath}' ${VIEWPORT.width}x${VIEWPORT.height} ` +
      ...
      `fov=${CONFIG.camera.projection.fovDgr} hair=${CONFIG.hairFile} fiber=${CONFIG.hairRender.fiberRadius} meshes=${DROP_MESHES ? 0 : 1}`
  );
}
