//
// index.plate.ts (from index.control.ts) — one groom, one display mode, one PNG, so it can be
...
import { HairFile } from './constants.ts';
...
// Hair albedo is an env knob here so the same plate can be rendered twice: once in the
// control's warm brown for looking at, and once in pure green as a SEGMENTATION key. Skin
// and background are both achromatic in this scene, so `G > R + margin` is an exact hair
// mask that no shadow, AO or tonemap step can forge.
CONFIG.hairRender.material.color0 = col(num('HR0', 59), num('HG0', 38), num('HB0', 27));
CONFIG.hairRender.material.color1 = col(num('HR1', 92), num('HG1', 63), num('HB1', 44));
...
// Which groom, which display mode. MODE 0 is the beauty frame; MODE 4 (USED_SLICES) writes
// the hairFinePass slice count per pixel, which is a direct read-out of BOTH the hair's
// screen coverage (any non-zero pixel is hair) and its depth complexity (the value itself).
CONFIG.hairFile = (Deno.env.get('HAIR') ?? CONFIG.hairFile) as HairFile;
CONFIG.displayMode = num('MODE', 0);
// Fibre radius, as a MULTIPLIER on the author's own value: the other lever on apparent
// density, and the one that does not cost per-strand work.
CONFIG.hairRender.fiberRadius *= num('RADIUS_MUL', 1);
CONFIG.hairSimulation.enabled = Deno.env.get('SIM') === '1';
