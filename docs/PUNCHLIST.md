...
- [ ] **0.1** Vite + three.js r185 scaffold; `WebGPURenderer` with WebGL2 fallback detection; DPR
      and resize handling. Gate: renders a lit sphere at 60 fps, reports backend in the UI.
- [ ] **0.2** Blender 4.2+ with MPFB2 + `faceunits01` installed headlessly. Gate:
      `blender --background --python` prints the 52 ARKit target names.
- [ ] **0.3** Figure pipeline: gender sweep → ARKit faceunits + OVR visemes → bake → GLB.
...
- [ ] **0.6** `tools/critic/measure.mjs` — headless capture + the six objective gates.
- [ ] **0.7** Blind A/B harness: shuffles ours vs reference, strips provenance, collects a verdict.
- [ ] **0.8** SPIKE: morph-target cost. 52 + 15 visemes + 2 gender on 13.7k verts. Sets the budget.
- [ ] **0.9** SPIKE: hair perf — frostbitten-hair demo on this Mac, built-in profiler, sweep strands.
- [ ] **0.10** SPIKE: RectAreaLight cost curve. Where does the portrait rig start hurting?
