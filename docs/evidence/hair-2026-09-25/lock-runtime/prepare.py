...
p = Path('tmp/hair-sep25/lock-runtime')
sources = ['tools/figure-pipeline/hair_tips.mjs', 'tools/figure-pipeline/hair_opacity.mjs',
           ...
           'node_modules/three/examples/jsm/tsl/display/TAAUNode.js', 'assets/hair/bob01/g050.glb']
...
for name in ['tips', 'opacity']:
    ...
    s = s.replace('async function screenshot(page, file) {',
                  "async function screenshot(page, file) {\n  await page.__auditLock(file);\n  if (page.__lockProblems?.length) throw Error(page.__lockProblems.join('\\n'));")
    if name == 'tips':
    ...
    if name == 'tips':
        s = s.replace('  const regions = { tips, curtain, mass, skin, backdrop };',
                      "  const regions = { tips, curtain, mass, skin, backdrop };\n  saveMasks(options.out, arm, raster, { ...regions, stable });")
    else:
        s = s.replace('  return {\n    footprintPixels:',
                      "  saveMasks(options.out, view.name, raster, { footprint, outside, mass, overFace, curtain, noInterior, carriesStep });\n  return {\n    footprintPixels:")
