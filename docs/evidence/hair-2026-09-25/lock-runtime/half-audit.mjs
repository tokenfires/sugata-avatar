...
export function unpackHalfReadback(data,width,height) {
  if(!(data instanceof Uint16Array))throw Error('Expected binary16 Uint16Array');
  const rowElements=width*4,strideElements=Math.ceil(width*8/256)*128;
  const expected=(height-1)*strideElements+rowElements;
  if(data.length!==expected)throw Error(`Unexpected padded RGBA16F length ${data.length}, expected ${expected}`);
  if(strideElements===rowElements)return {data,strideBytes:strideElements*2,paddingBytes:0};
  const packed=new Uint16Array(width*height*4);
  for(let y=0;y<height;y++)packed.set(data.subarray(y*strideElements,y*strideElements+rowElements),y*rowElements);
  return {data:packed,strideBytes:strideElements*2,paddingBytes:(strideElements-rowElements)*2};
}
