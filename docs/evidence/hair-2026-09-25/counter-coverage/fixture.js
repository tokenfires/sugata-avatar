...
async function draw() {
  ...
  previous = current;
  return {bytes, oracleMismatches, counters:cards.map(c=>c.material.counterUniform?.value??null), mean: sum / mask.length / 255, min: min / 255, max: max / 255,
  ...
  async run(count = 512) {
    ...
    for (let n = 0; n < count; n++) {
      ...
      if ([24, 128, 512].includes(n + 1)) {
        ...
        checkpoints.push({native:await auditNative(),count:n+1, temporalMean:frames.reduce((s,f)=>s+f.mean,0)/frames.length,
