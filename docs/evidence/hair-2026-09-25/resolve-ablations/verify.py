...
identities=[];rows=[];per_pixel=[];control_failures=[]
for r in results:
 ...
 if c['name'] in ['empty','opaque-pair']:
  ...
  bad=[{'frameId':f['frameId'],'min':f['min'],'max':f['max'],'mean':f['mean'],'nativeMean':f['resolve']['values']['roi']['redMean']} for f in r['frames'] if not f['min']==f['max']==value]
  if bad:control_failures.append({'case':name(r),'control':'exact endpoint','expected':value,'failedFrames':len(bad),'frames':bad})
...
(root/(run+'-control-failures.json')).write_text(json.dumps(control_failures,indent=2)+'\n')
for failure in control_failures:print('FAIL:',failure['case'],failure['control'],str(failure['failedFrames'])+' frames', 'first',failure['frames'][0])
if not control_failures:print('PASS: exact endpoint controls.')
raise SystemExit(1 if control_failures else 0)
