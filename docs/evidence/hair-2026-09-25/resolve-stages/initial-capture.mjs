...
const root = 'tmp/hair-sep25/resolve-stages';
const cases = [
...
const report={baseCommit:'1d680b5f240e3b3f4346c154081d69b4df96652d',
...
try {
  ...
  for(const diagnostic of diagnostics) for(const temporal of modes) for(const item of cases.filter(c=>!select||select.includes(c.name))) for(const phase of item.phases??[0]) {
    ...
    const config={mode:'stochastic',sampler:'counter',...settings,temporal,phase,diagnostic};
    ...
    try {
      ...
      for(const f of data.frames){
        ...
        if(diagnostic==='identity')assert.equal(f.diagnostic,null);
        else assert.equal(f.diagnostic.values.whole.nan+f.diagnostic.values.whole.infinity,0);
      ...
      for(const checkpoint of data.checkpoints)if(temporal==='taau'){
        ...
        for(const t of native.targets){
          ...
          assert.equal(t.attachments,t.name==='resolve'?(diagnostic==='identity'?1:3):2);
