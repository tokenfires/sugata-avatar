...
import { Color, Mesh, MeshBasicNodeMaterial, PlaneGeometry, RenderTarget, Vector3,
  NoColorSpace, NoToneMapping, UnsignedByteType, HalfFloatType } from 'three/webgpu';
import { interleavedGradientNoise, screenCoordinate, vec4 } from 'three/tsl';
...
cards.forEach((card, i) => {
  ...
  if (config.mode === 'stochastic') {
    ...
    // Rebuild the same expression for identity controls; only the second separate
    // material receives yx in the spatial candidate. No production shader is edited.
    const swapped = config.spatial === 'swapped' && i === 1;
    const coordinate = swapped ? screenCoordinate.yx : screenCoordinate.xy;
    card.material.alphaTestNode = interleavedGradientNoise(coordinate)
      .add(card.material.hairDitherOffset).fract().clamp(1e-6, 1);
    card.material.needsUpdate = true;
    card.userData.spatialField = swapped ? 'yx' : 'xy';
  }
  stage.add(card);
});
if (config.frontBody) {
...
let previous;
const resolved=stage.temporal?.node.passNode;
if(resolved?.constructor.name!=='TAAUNode')throw Error('Unexpected temporal owner: '+resolved?.constructor.name);
if(stage.temporal.sharpenNode!==null)throw Error('Unexpected sharpen node');
let auditMetadata, nativeResolveBytes;
async function readNative() {
  const targets=[['resolve',resolved._resolveRenderTarget,0],
    ['historyColor',resolved._historyRenderTarget,0],
    ...(resolved._resolveRenderTarget.textures.length>1?[['resolveLock',resolved._resolveRenderTarget,1]]:[]),
    ['historyLock',resolved._historyRenderTarget,1]];
  const result={};let resolveBytes,resolveLockBytes;
  auditMetadata={owner:resolved.constructor.name,sharpenNode:false,targets:[]};
  for(const [name,rt,index] of targets){
    const texture=rt.textures[index];
    if(texture.type!==HalfFloatType||rt.width!==size||rt.height!==size)throw Error('Unexpected native target');
    const data=await renderer.readRenderTargetPixelsAsync(rt,0,0,rt.width,rt.height,index);
    auditMetadata.targets.push({name,index,width:rt.width,height:rt.height,attachments:rt.textures.length,
      textureType:texture.type,textureFormat:texture.format,colorSpace:texture.colorSpace,
      gpuFormat:renderer.backend.get(texture).texture?.format??null,typedArray:data.constructor.name});
    result[name]=auditHalf(data,rt.width,rt.height,bounds);
    if(name==='resolve'){resolveBytes=data;nativeResolveBytes=data;}
    if(name==='resolveLock')resolveLockBytes=data;
    if(name==='historyLock'&&resolveLockBytes){
      let differences=0;
      for(let i=0;i<data.length;i++)if(data[i]!==resolveLockBytes[i])differences++;
      result.lockHistoryBitDifferences=differences;
    }
    if(name==='historyColor'){
      let differences=0;
      for(let i=0;i<data.length;i++)if(data[i]!==resolveBytes[i])differences++;
      result.resolveHistoryBitDifferences=differences;
    }
  }
  return result;
}
async function draw() {
  ...
  const nativeAudit = await readNative();
  ...
  // Bind the decoded native color to this frame's ordinary image. Nonfinite native
  // values stay separately counted; a cast to Uint8 must not hide them here.
  let maxQuantizationError=0,quantizationMismatches=0,finiteCompared=0;
  for(let i=0;i<bytes.length;i++){
    const value=decodeHalf(nativeResolveBytes[i]);
    if(!Number.isFinite(value))continue;
    const expected=Math.round(Math.max(0,Math.min(1,value))*255);
    const error=Math.abs(bytes[i]-expected);
    maxQuantizationError=Math.max(maxQuantizationError,error);
    if(error>1)quantizationMismatches++;
    finiteCompared++;
  }
  nativeAudit.quantization={finiteCompared,maxQuantizationError,quantizationMismatches};
  ...
  previous = current;
  return {bytes, nativeAudit, mean: sum / mask.length / 255, min: min / 255, max: max / 255,
  ...
  async run(count = 512) {
    ...
    for (let n = 0; n < count; n++) {
      ...
      frames.push({frameId: last.frameId, mean: last.mean, min: last.min, max: last.max,
        spatialSD: last.spatialSD, temporalRMS: last.temporalRMS, nativeAudit:last.nativeAudit, offsets: last.offsets});
    ...
    return {config, actual: {stats: stage.stats, adapter: adapterInfo, frameId: frame.frameId,
      captureTime: frame.time, nativeAudit:auditMetadata, materials: cards.map(c => ({transparent:c.material.transparent,
        depthWrite:c.material.depthWrite, alphaTest:c.material.alphaTest, alphaTestNode:!!c.material.alphaTestNode,
        triangles:c.geometry.index.count/3, renderOrder:c.renderOrder, rate:c.userData.ditherRate??null, field:c.userData.spatialField??null}))},
