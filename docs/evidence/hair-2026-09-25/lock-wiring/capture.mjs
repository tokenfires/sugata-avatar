...
const root = 'tmp/hair-sep25/lock-wiring';
const cases = [
...
const arms=process.env.ARMS?.split(',')??['identity','attachment-only','copy-lock'];
...
const sources=['packages/core/src/render/HairOIT.js','packages/core/src/render/Stage.js',
  ...
  'assets/hair/bob01/g050.glb',root+'/fixture.js',root+'/capture.mjs',root+'/half-audit.mjs',root+'/route.mjs',
  'node_modules/three/examples/jsm/tsl/display/TAAUNode.js'];
const sha = data => createHash('sha256').update(data).digest('hex');
const report={baseCommit:'c55b923d756a3d911567796b8b133e405eee1f11',
...
try {
  ...
  for(const wiring of arms) for(const temporal of modes) for(const item of cases.filter(c=>!select||select.includes(c.name))) for(const phase of item.phases??[0]) {
    const {phases,...settings}=item;
    const config={mode:'stochastic',sequence:'shared',spatial:'swapped',...settings,temporal,phase,wiring};
    ...
    try {
      ...
      const saveSource=(file,text)=>{
        const target=path.join(out,file);
        if(fs.existsSync(target))assert.equal(sha(fs.readFileSync(target)),sha(text));
        else fs.writeFileSync(target,text);
      };
      await page.route(/\/(?:three_addons_tsl_display_TAAUNode__js|TAAUNode)\.js(?:\?|$)/,async route=>{
        const response=await route.fetch();
        const before=await response.text(),after=patch(before,wiring);
        saveSource('original-served.js',before);saveSource(wiring+'-served.js',after);
        routes.push({url:route.request().url(),arm:wiring,beforeSHA256:sha(before),afterSHA256:sha(after)});
      ...
      assert.deepEqual(errors,[]);assert.equal(routes.length,1,'TAAU route not applied exactly once');
      data.routes=routes;
      assert.equal(data.actual.nativeAudit.targets.find(t=>t.name==='resolve').attachments,wiring==='identity'?1:2);
      ...
      for(const f of data.frames){
        assert.equal(f.nativeAudit.resolveHistoryBitDifferences,0);
        assert.equal(f.nativeAudit.quantization.quantizationMismatches,0);
        if(wiring==='copy-lock')assert.equal(f.nativeAudit.lockHistoryBitDifferences,0);
      }
      ...
      const name=`${wiring}-${temporal}-${item.name}-p${phase}`;
      ...
      const maxNonfinite=Math.max(...data.frames.flatMap(f=>Object.values(f.nativeAudit).filter(v=>v?.whole).map(v=>v.whole.nan+v.whole.infinity)));
      const maxLock=Math.max(...data.frames.map(f=>f.nativeAudit.historyLock.whole.finiteMax));
      console.log(name+' maxNonfinite='+maxNonfinite+' maxLock='+maxLock+' '+data.checkpoints.map(c=>`n${c.count}:mean=${c.temporalMean.toFixed(5)},tail=${c.tailMean.toFixed(5)},final=${c.finalMean.toFixed(5)},temporalRMS=${c.tailTemporalRMS.toFixed(5)}`).join(' | '));
