...
const root = 'tmp/hair-sep25/spatial-fields';
const cases = [
  ...
  ...['shared','distinct'].flatMap(sequence => [
    ['quarter',[.25,.25]], ['half',[.5,.5]], ['three-quarter',[.75,.75]], ['mixed',[.25,.75]]
  ].map(([name,alphas]) => ({name:`swapped-${sequence}-${name}`,alphas,sequence,phases:[0,977]}))),
  {name:'reversed-shared',alphas:[.5,.5],reverse:true},
  {name:'reversed-distinct',alphas:[.5,.5],reverse:true,sequence:'distinct'},
...
const report={baseCommit:'b60ed01be58550cd222a0c8d6b9b093f9f7a3849',
