...
2. Develop the promising attention prototype into an explicitly experimental, user-invoked action aimed at the actual viewer, with owned interruption/release/cancel behavior. Resolve remains open: compare a gaze hold alone or moderate eye-opening attenuation without the rejected sustained squint/brow profile. Resolve the reported coy/teasing ambiguity without guessing its source. Each actionable critique needs two alternatives and visual or behavioral evidence.
