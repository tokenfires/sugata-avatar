...
### R20, added by the user 2026-08-17 — the scene the avatar stands in

> If you accompish hair, move on to scene setting and building so he AI and users can provide an
> environment for their avatar. Think typical situations through someone's day but also include fun
> scenes like vacations, going to the beach, out to dinner, etc. We don't need a huge corpus here but
> its important you understand the intent so you build into the goal.

**This is an omission in the same family as R18 and R19, and the same argument carries.** R7 asks
for a system "usable by any AI agent to embody itself." Until now the only place an embodied AI
could stand was a photographic studio: `BACKGROUND_PRESETS` offers `studio`, `void` and a refused
`transparent`, and a "scene" is one emissive card at `BACKDROP_DISTANCE_METRES` 1.9 with a ground
contact shadow. That is a product decision nobody made — it is what a *rendering* phase produces
when nothing asks it for a *place*.

🎯 **Read the intent, because it is not "ship a level editor."** The user's own words are
"typical situations through someone's day" first and "vacations, beach, dinner" second, and the
order is the design. A person's day is mostly the same four or five rooms; the memorable scenes are
rare and are memorable *because* they are rare. An avatar that can only ever appear on a beach is as
wrong as one that can only appear in a studio. The corpus is deliberately small — "we don't need a
huge corpus here" is a constraint, not a hedge — so the value has to come from **a scene being a
composable description rather than a downloaded set**.

⚠️ **Two constraints are already binding and shape this before any art is authored:**

- **The LFS budget.** Assets ship through git-LFS against a free tier of 1 GB storage and 1 GB/month
  bandwidth, and `assets/` is already 232 MB — roughly four clones a month. A scene system built
  from downloaded HDRIs and set meshes would exhaust that on its first three scenes. **Scenes must
  be procedural and parametric by default**, generated at load or baked small, in the same way the
  groom is generated by `hair_cards.py` rather than modelled. This is the standing "build it
  ourselves" preference arriving as an actual engineering constraint rather than a taste.
- **The calibrated gates.** `report().scene.lighting.calibrated` goes `false` the moment `exposure`
  or `ambient` leave 1, because every committed gate number in `docs/PROGRESS.md` was measured under
  the studio rig. A scene changes the light by definition. So a scene must either carry its own
  declared gate baseline or state that it invalidates the studio ones — silently re-lighting the
  figure would retire the whole measured record without a single failing test.

**And the thing R20 buys that no rendering work can:** the blind critic has spent rounds reporting
lighting nits because a bald nude figure on a grey card is all it was ever shown. A figure with
hair, clothes and a *place* is the first subject in this project's history that a harsh critic could
judge the way it judges a game.
...
| R20 | **Scene setting** — the AI and the user can give the avatar an environment: the ordinary places of a day first, and occasional scenes (vacation, beach, dinner) second. Small corpus, understood intent | Phase 11, `research/scene-system.md` |
